...
### Active Days (implemented)

- Lets the user choose which weekdays GETUPP arms (presets: Every day / Weekdays / Weekends, plus full manual picking). Full spec in `PRDs/active-days.md`. Production-tier code (same tier as the blocking module, Timeout, and Streak) — the store, the scheduling predicate, and the cross-midnight session logic survive into production.
- **No mode enum, ever.** `activeDays: Set<Int>` (Calendar weekday numbers, 1 = Sunday … 7 = Saturday) in App Group UserDefaults is the single source of truth. "Weekdays"/"Weekends"/"Every day" labels and chip-lit state are DERIVED on read (`ActiveDays.preset(for:)`, `ActiveDays.detailLabel(...)`) — same principle as `Streak`/`Timeout`. A hand-picked Mon–Fri correctly lights "Weekdays"; there's no separate mode to disagree with the set. `activeDays` is never empty on disk: absent/empty/invalid on read defaults to all 7 with an error breadcrumb (`ActiveDays.load()`).
- `activeDays` is stored OUTSIDE the `wakeSchedule` JSON blob — a plain `[Int]` array key, so any Foundation-only process can read/write it without a JSON round-trip. `WakeSchedule` now owns the TIME window only (`isWindowActive`/`nextWindowStart` take `activeDays` as a parameter); the old `DayMode`/`customDays` fields are gone (old persisted blobs decode fine — `JSONDecoder` ignores unknown keys).
- **Same-day downgrade queue** (mirrors Timeout R5 — no same-day dodge): removing TODAY while today's window hasn't ended queues to `pendingActiveDays` (+ `pendingActiveDaysSetDate`), promoted at the next `ActiveDays.scheduleMaintenance()`. Additions and future-day removals apply immediately and cancel any queued removal. Save algorithm is the pure `ActiveDays.resolveSave(...)`.
- **Scheduling engine:** still ONE daily `DeviceActivitySchedule` — NOT seven per-weekday activities (multiplies the flaky-callback surface for no benefit). The monitor's `intervalDidStart` runs `Timeout.dailyMaintenance()` then `ActiveDays.scheduleMaintenance()` (promotion must happen BEFORE the gate — it's what can make today unscheduled), then no-ops entirely on an unscheduled day: no shield, no `recordSessionRanToday`, no `activeSessionDate` write. `ShieldManager.reconcileState()` re-checks the same predicate on every app-open (check-on-open layer) so a missed/stale monitor read self-heals. `WakeSchedule.nextWindowStart()` is a bounded 8-day forward search with a defined answer even for an empty set (defensive: treated as all 7) — never nil unless the schedule itself is disabled.
- **Streak semantics:** an unscheduled day is a skip — doesn't break the streak, doesn't count toward it (`DayRecord.wasScheduled`, already existed pre-feature; `resolvePastState`/`resolveTodayState` already read it as `.rest`). The days-off compensation for "doesn't break you" is "doesn't build you."
- **Cross-midnight session ownership (R6):** a session belongs to the calendar day its window STARTED, not the day it resolves. `activeSessionDate` (App Group key, "yyyy-MM-dd") is written at window arm and is the anchor every session-lifecycle write uses instead of `Date()` — `markVerifiedToday`, `markEmergencyUsedToday`, `recordSessionRanToday` all anchor to it via `GetuppShared.sessionKey()`. `Timeout.beginTimeout()` inherits it (does NOT re-derive — don't "fix" this). `Timeout.completeTimeoutIfElapsed()` clears it as the finalization latch (same shape as the `timeoutEndTime` latch) — it can't write a `DayRecord` directly since `Timeout.swift` is Foundation-only and runs in the shield-action extension; the `verified` flag was already written correctly at photo time (verification always happens before midnight — windows can't cross it), so clearing the key IS the credit. `GetuppShared.removeShield()` also clears it (window-end, missed-callback reconcile, escape hatch, debug unlock all funnel through here). `Streak.deriveStreak()` takes `activeSessionDate` and holds that past date `.pending` while the key is set — no premature +1, no break. `GetuppShared.backfillDayLog()` skips the day the key points at while it's open.
  - **Ordering rule that looks redundant but isn't:** `ShieldManager.emergencyBreak()`/`pullThePlug()` call `markEmergencyUsedToday()` BEFORE `wipeTimeoutAndUnshield()` — since `removeShield()` clears `activeSessionDate`, marking after the wipe would break a post-midnight emergency break (it'd mark today instead of the actual session day).
  - **Wake window picker already enforces no-cross-midnight** (end must be after start, same day) via the existing `durationMinutes > 0` validation in `WakeWindowEditSheet` — this is what makes R6 well-defined in the first place.
- **Empty-state guard (deactivation funnel):** unticking the last day disables Save and shows "Zero days is just GETUPP off." + a button that dismisses the Days screen WITHOUT saving and lands on the Escape Hatch hub — no auto-toggle. Deliberate friction, consistent with why Escape Hatch options are hidden behind a tap.
- **Settings IA:** minimal — a SCHEDULE card holds the existing Wake Window row plus a new Days row (`NavigationLink(isActive:)`, right-detail = `ActiveDays.detailLabel(...)`). Timeout/Apps/Escape Hatch cards untouched (full three-section IA restructure from the PRD sketch was deferred — cosmetic, not P0).
- **"Next: Monday" hint (P1, shipped):** `ContentView.idleView` shows a caption line only when today is unscheduled, so a quiet morning doesn't read as broken.
- **Target membership (manual, in Xcode):** `ActiveDays.swift` → Getupp + GetuppMonitor (never the shield targets — it leans on `GetuppShared`/`Streak` helpers those targets don't link; shields only run downstream of a shield an unscheduled day can never create). `ActiveDaysView.swift` → Getupp only. Self-tests: `runActiveDaysSelfTests()` in `ActiveDays.swift` (presets, week ordering, label derivation incl. no-wraparound, `resolveSave`), plus new R6/skip fixtures appended to `runStreakSelfTests()` in `Streak.swift`, plus one OQ1 clamp fixture in `runTimeoutSelfTests()`. All reachable from Internal Tools' "Active Days" debug section.
- **Known limitation (accepted for v1):** if a user edits their schedule and doesn't open the app for a week, `backfillDayLog` stamps the elapsed gap days using the CURRENT `activeDays`, not whatever set governed them at the time. Self-limiting (maintenance runs every foreground).
